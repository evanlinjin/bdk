...
impl Mnemonic {
    ...
    pub fn parse_in(language: Language, mnemonic: &str) -> Result<Mnemonic, Error> {
        ...
        let language_word_map = language.word_map();

        // parse word indexes and ENT+CS bits
        let (words, ent_cs_bits) = split_mnemonic
            .iter()
            .map_while(|&w| language_word_map.get(w))
            .fold(
                (Vec::new(), Vec::new()),
                |(mut words, mut ent_cs), word_index| {
                    words.push(*word_index);
                    ent_cs.extend(utils::u11_to_bool_array(word_index));
                    (words, ent_cs)
                },
            );

        // check that we have parsed all words, otherwise return error on failed index
        if words.len() != ms {
