...
impl Wallet {
    ...
    fn get_new_address(&mut self, keychain: KeychainKind) -> AddressInfo {
        ...
        self.spk_tracker
            .add_spk((keychain, next_index), definite_descriptor.script_pubkey());

        let address = definite_descriptor
    ...
    fn get_unused_address(&mut self, keychain: KeychainKind) -> AddressInfo {
        let next_script = self
            .spk_tracker
            .iter_unused()
            .find(|((kc, _), _)| kc == &keychain)
            .map(|((_, index), spk)| (index, spk));

        match next_script {
            Some((index, next_script)) => AddressInfo {
                address: Address::from_script(next_script, self.network)
                    .expect("descriptor must have address form"),
                index,
                keychain,
            },
    ...
    pub fn is_mine(&self, script: &Script) -> bool {
        self.spk_tracker.index_of_spk(script).is_some()
    }
    ...
    pub fn get_tx(&self, txid: Txid, include_raw: bool) -> Option<TransactionDetails> {
        let height = self.sparse_chain.transaction_height(txid)?;
        let tx = self.tx_graph.tx(txid).expect("must be in graph").clone();
        ...
        let sent = tx
            ...
            .iter()
            .map(|txin| {
                if self.spk_tracker.txout(txin.previous_output).is_some() {
                    self.tx_graph
                        .txout(txin.previous_output)
                        .expect("must be in graph")
                        .value
                } else {
                    0
                }
        ...
        Some(TransactionDetails {
            transaction: if include_raw { Some(tx) } else { None },
            txid,
            received,
            sent,
            fee,
    ...
    pub fn get_balance(&self) -> Balance {
        ...
        let last_sync_height = match self.sparse_chain.latest_checkpoint() {
    ...
    pub fn build_fee_bump(
        ...
        let tx_and_height = self
            .tx_graph
            .tx(txid)
            .and_then(|tx| Some((tx.clone(), self.sparse_chain.transaction_height(txid))));
        let mut tx = match tx_and_height {
            None => return Err(Error::TransactionNotFound),
            Some((tx, Some(TxHeight::Confirmed(_)))) => return Err(Error::TransactionConfirmed),
            Some((tx, _)) => tx,
        };
        ...
        let original_utxos = original_txin
            ...
            .map(|txin| -> Result<_, Error> {
                ...
                let (weight, keychain) = match self.spk_tracker.index_of_spk(&txout.script_pubkey) {
                    Some((keychain, _)) => (
        ...
        if tx.output.len() > 1 {
            ...
            for (index, txout) in tx.output.iter().enumerate() {
                ...
                match self.spk_tracker.index_of_spk(&txout.script_pubkey) {
    ...
    pub fn finalize_psbt(
        ...
        for (n, input) in tx.input.iter().enumerate() {
            ...
            let confirmation_height = self
                .sparse_chain
                .transaction_height(input.previous_output.txid)
                .map(|height| match height {
                    TxHeight::Confirmed(height) => height,
                    TxHeight::Unconfirmed => u32::MAX,
                });
    ...
    fn get_descriptor_for_txout(&self, txout: &TxOut) -> Option<DerivedDescriptor> {
        let (keychain, child) = self.spk_tracker.index_of_spk(&txout.script_pubkey)?;
                            ...
                            if let Some(current_height) = current_height {
                                match confirmation_time {
                                    TxHeight::Confirmed(h) => {
                                        ...
                                        spendable &=
                                            (current_height.saturating_sub(h)) >= COINBASE_MATURITY;
                                    }
                                    TxHeight::Unconfirmed => spendable = false,
                                }
                            }
                        }
                        spendable
    ...
    pub fn get_psbt_input(
        ...
        if let Some(prev_tx) = self.tx_graph.tx(prev_output.txid) {
    ...
    fn update_psbt_with_descriptor(
        ...
        for (is_input, index, out) in utxos.into_iter() {
            if let Some((keychain, child)) = self.spk_tracker.index_of_spk(&out.script_pubkey) {
                debug!(
...
pub(crate) mod test {
    ...
    //     fn test_spend_from_wallet(wallet: Wallet) {
    //         let addr = wallet.get_address(AddressIndex::New).unwrap();
