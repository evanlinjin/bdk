...
fn test_get_chain_position() {
    ...
    use bdk_chain::{Anchor, BlockTime, SpkTxOutIndex};

    struct TestCase<AM> {
        name: &'static str,
        tx: Transaction,
        anchor: Option<(Anchor, AM)>,
        last_seen: Option<u64>,
        exp_pos: Option<ChainPosition<BlockTime>>,
    }
    ...
    fn run(
        chain: &LocalChain,
        graph: &mut IndexedTxGraph<BlockTime, SpkTxOutIndex<u32>>,
        test: TestCase<BlockTime>,
    ) {
        let TestCase {
            name,
            tx,
        ...
        if let Some(((_, blockid), anchor_meta)) = anchor {
            let _ = graph.insert_anchor((txid, blockid), anchor_meta);
        }
    ...
    [
        ...
        TestCase {
            ...
            last_seen: None,
            exp_pos: Some(ChainPosition::Confirmed(anchors[1])),
        },
        TestCase {
