...
use bdk_core::{BlockUpdate, TxUpdate};
...
use crate::{
    indexer::{FilteringIndexer, Index, Indexer},
    spk_txout::SpkTxOutIndex,
    tx_graph::{self, TxGraph},
    Anchor, BlockId, ChainOracle, Merge, TxPosInBlock,
};
...
#[derive(Debug, Clone)]
pub struct IndexedTxGraph<A, X> {
    /// Transaction index.
    index: X,
    graph: TxGraph<A>,
}

impl<A, X: Default> Default for IndexedTxGraph<A, X> {
    fn default() -> Self {
        Self {
...
impl<A, X> IndexedTxGraph<A, X> {
    ...
    /// Get a reference of the internal indexer.
    pub fn indexer(&self) -> &X {
        &self.index
    }
}

impl<A: Anchor, X: Index<A>> IndexedTxGraph<A, X> {
    /// Mutate the internal indexer.
    pub fn mutate_indexer<F, R>(&mut self, mut f: F) -> (R, ChangeSet<A, X::ChangeSet>)
    where
        F: FnMut(&mut X, &mut X::ChangeSet) -> R,
    {
        let mut changeset = X::ChangeSet::default();
        let result = f(&mut self.index, &mut changeset);
        self.index.reindex(&self.graph.clone().into());
        (
            result,
            ChangeSet {
                indexer: changeset,
    ...
    /// Mutate the internal indexer.
    ///
    /// Forces a re-index of `TxGraph` data after mutation.
    pub fn indexer_mut<F, R>(&mut self, mut f: F) -> R
    where
        F: FnMut(&mut X) -> R,
    {
        let result = f(&mut self.index);
        self.index.reindex(&self.graph().clone().into());
        result
    }
    ...
    pub fn initial_changeset<U>(&self) -> ChangeSet<A, X::ChangeSet> {
...
impl<A: Anchor, X: Index<A>> IndexedTxGraph<A, X> {
    ...
    pub fn apply_update<U>(&mut self, update: U) -> ChangeSet<A, X::ChangeSet>
    where
        U: Into<TxUpdate<A>>,
        X: Indexer<A, U>,
    {
        let mut changeset = ChangeSet::<A, X::ChangeSet>::default();
        self.index.index(&mut changeset.indexer, &update);
        changeset
            .tx_graph
            .merge(self.graph.apply_update(update.into()));
        changeset
    }

    /// Filters and applies an `update`.
    pub fn filter_and_apply_update<U>(&mut self, update: U) -> ChangeSet<A, X::ChangeSet>
    where
        X: FilteringIndexer<A, U>,
    {
        let mut changeset = ChangeSet::<A, X::ChangeSet>::default();
        let tx_update = self.index.index_and_filter(&mut changeset.indexer, &update);
        changeset.tx_graph.merge(self.graph.apply_update(tx_update));
        changeset
    }

    /// Indexes and insert a `txout` of given `outpoint`.
    pub fn insert_txout(&mut self, outpoint: OutPoint, txout: TxOut) -> ChangeSet<A, X::ChangeSet>
    where
        X: Indexer<A>,
    {
        let mut update = TxUpdate::<A>::default();
        update.txouts.insert(outpoint, txout.clone());
        self.apply_update(update)
    }

    /// Indexes and inserts a transaction into the graph.
    pub fn insert_tx(&mut self, tx: impl Into<Arc<Transaction>>) -> ChangeSet<A, X::ChangeSet>
    where
        X: Indexer<A>,
    {
        let mut update = TxUpdate::<A>::default();
        update.txs.push(tx.into());
        self.apply_update(update)
    }
    ...
    pub fn insert_evicted_at(&mut self, txid: Txid, evicted_at: u64) -> ChangeSet<A, X::ChangeSet> {
...
where
    ...
    for<'b> X: FilteringIndexer<A, BlockUpdate<'b>>,
{
    ...
    pub fn apply_block(&mut self, block: &Block, height: u32) -> ChangeSet<A, X::ChangeSet> {
        self.filter_and_apply_update(BlockUpdate::new(block, height))
