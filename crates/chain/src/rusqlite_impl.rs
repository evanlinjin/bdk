...
impl tx_graph::ChangeSet<ConfirmationBlockTime> {
    ...
    pub fn schema_v2() -> String {
        format!(
            "ALTER TABLE {} ADD COLUMN last_missing INTEGER",
            Self::TXS_TABLE_NAME,
        )
    }
    ...
    pub fn from_sqlite(db_tx: &rusqlite::Transaction) -> rusqlite::Result<Self> {
        ...
        let mut statement = db_tx.prepare(&format!(
            "SELECT txid, raw_tx, last_seen, last_missing FROM {}",
            Self::TXS_TABLE_NAME,
        ))?;
        let row_iter = statement.query_map([], |row| {
            Ok((
                ...
                row.get::<_, Option<u64>>("last_missing")?,
            ))
        })?;
        for row in row_iter {
            let (Impl(txid), tx, last_seen, last_missing) = row?;
            ...
            if let Some(last_missing) = last_missing {
                changeset.last_missing.insert(txid, last_missing);
            }
        }
    ...
    pub fn persist_to_sqlite(&self, db_tx: &rusqlite::Transaction) -> rusqlite::Result<()> {
        ...
        let mut statement = db_tx
            .prepare_cached(&format!(
                "INSERT INTO {}(txid, last_missing) VALUES(:txid, :last_missing) ON CONFLICT(txid) DO UPDATE SET last_missing=:last_missing",
                Self::TXS_TABLE_NAME,
            ))?;
        for (&txid, &last_missing) in &self.last_missing {
            let checked_time = last_missing.to_sql()?;
            statement.execute(named_params! {
                ":txid": Impl(txid),
                ":last_missing": Some(checked_time),
            })?;
        }
...
mod test {
    ...
    #[test]
    fn can_persist_last_missing() -> anyhow::Result<()> {
        ...
        let last_missing = 100;

        // Persist `last_missing`
        {
            let changeset = ChangeSet {
                last_missing: [(txid, last_missing)].into(),
                ..Default::default()
            };
            let db_tx = conn.transaction()?;
            changeset.persist_to_sqlite(&db_tx)?;
        ...
        // Load from sqlite should succeed
        {
            ...
            let changeset = ChangeSet::from_sqlite(&db_tx)?;
            db_tx.commit()?;
            assert_eq!(changeset.last_missing.get(&txid), Some(&last_missing));
        }
