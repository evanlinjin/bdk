...
use std::prelude::rust_2021::{String, ToString, Vec};
use std::sync::Arc;
// use rusqlite;
use crate::{keychain_txout, local_chain, tx_graph, Anchor, DescriptorId};
    ...
    A: Anchor + Clone + Ord + serde::Serialize + serde::de::DeserializeOwned,
{
    ...
    async fn init_postgres_tables(
        ...
        // sqlx::migrate!("./tx_graph_migrations").run(db_tx).await
        Ok(())
    }
...
impl local_chain::ChangeSet {
    ...
    async fn init_postgres_tables(
        ...
        // sqlx::migrate!("./local_chain_migrations")
        //     .run(&mut **db_tx)
        //     .await
        Ok(())
    }
...
impl keychain_txout::ChangeSet {
    ...
    async fn init_postgres_tables(
        ...
        // sqlx::migrate!("./keychain_migrations")
        //     .run(&mut **db_tx)
        //     .await
        Ok(())
    }
