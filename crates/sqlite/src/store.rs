...
impl<S, P: Debug> Debug for Store<S, P> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        Debug::fmt(&(&self.conn, &self.params), f)
    }
}

impl<S: Storable> Store<S, S::Params> {
    /// Creates a new store from a [`Connection`].
    pub fn new(mut conn: Connection, params: S::Params) -> rusqlite::Result<Self> {
...
// impl<L, A> Store<L, A>
// where
//     L: Ord + for<'de> Deserialize<'de> + Serialize + Send,
//     A: Anchor + Send,
// {
//     fn insert_keychain(
//         db_transaction: &rusqlite::Transaction,
...
//         let label_json = serde_json::to_string(&label).expect("keychain json");
//         let keychain_id = keychain.descriptor_id().to_byte_array();
//         let keychain_str = keychain.to_string();
//         insert_keychain_stmt.execute(named_params! {":keychain": label_json, ":descriptor": keychain_str, ":descriptor_id": keychain_id })
//                 .map_err(Error::Sqlite)?;
//         Ok(())
//     }
//
//     fn set_keychain_last_revealed(
//         db_transaction: &rusqlite::Transaction,
...
//         let descriptor_id = keychain_id.to_byte_array();
//         update_last_revealed_stmt
//             .execute(
//                 named_params! {":descriptor_id": descriptor_id, ":last_revealed": last_revealed },
//             )
//             .map_err(Error::Sqlite)?;
//         Ok(())
//     }
...
//     fn select_keychains(
//         db_transaction: &rusqlite::Transaction,
//     ) -> Result<BTreeMap<L, Descriptor<DescriptorPublicKey>>, Error> {
...
//                 let keychain = serde_json::from_str::<L>(keychain.as_str()).expect("keychain");
...
impl<S: Storable> Store<S, S::Params> {
    ...
    pub fn write(&mut self, changeset: S) -> rusqlite::Result<()> {
        let db_tx = self.conn.transaction()?;
        changeset.write(&db_tx, &self.params)?;
        db_tx.commit()

        // Self::insert_keychain(&db_transaction, label, keychain)
        //
        ...
        // db_tx.commit().map_err(Error::Sqlite)
    }

    /// Read the entire database and return the aggregate [`CombinedChangeSet`].
    pub fn read(&mut self) -> rusqlite::Result<Option<S>> {
        S::read(&self.conn.transaction()?, &self.params)

        // let network = Self::select_network(&db_tx)?;
        // let chain = Self::select_blocks(&db_tx)?;
        // let keychains_added = Self::select_keychains(&db_tx)?;
        // let last_revealed = Self::select_last_revealed(&db_tx)?;
        // let txs = Self::select_txs(&db_tx)?;
        // let last_seen = Self::select_last_seen(&db_tx)?;
        // let txouts = Self::select_txouts(&db_tx)?;
        // let anchors = Self::select_anchors(&db_tx)?;
        //
...
/// Migration plan for a set of tables that are upgraded/migrated together.
pub struct MigrationPlan {
    /// Schemas by version.
    pub all_schemas: &'static [&'static str],

    /// Get the current schema version from database.
    pub get_current_schema_version:
        &'static dyn Fn(&rusqlite::Transaction) -> rusqlite::Result<u32>,

    /// Save the current schema version to database.
    pub set_current_schema_version:
        &'static dyn Fn(&rusqlite::Transaction, u32) -> rusqlite::Result<()>,
}

pub trait Storable: Sized {
    type Params: Default;

    /// Where tables are initialized.
    fn init(db_tx: &rusqlite::Transaction, params: &Self::Params) -> rusqlite::Result<()>;

    fn read(db_tx: &rusqlite::Transaction, params: &Self::Params)
        -> rusqlite::Result<Option<Self>>;

    fn write(&self, db_tx: &rusqlite::Transaction, params: &Self::Params) -> rusqlite::Result<()>;
}
...
//     // ) -> (
//     //     Vec<WalletChangeSet<Keychain, A>>,
//     //     WalletChangeSet<Keychain, A>,
//     // ) {
...
//     //
//     //     changesets.push(WalletChangeSet {
...
//     //         changesets
//     //             .iter()
//     //             .fold(WalletChangeSet::<Keychain, A>::default(), |mut i, cs| {
//     //                 i.append(cs.clone());
//     //                 i
//     //             });
//     //
